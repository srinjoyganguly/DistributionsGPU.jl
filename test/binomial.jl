...
@test DistributionsGPU.expectation(Binomial(6), identity) ≈ 3.0
@test DistributionsGPU.expectation(Binomial(10, 0.2), x->-x) ≈ -2.0

# Test mode
@test DistributionsGPU.mode(Binomial(100, 0.4)) == 40
@test DistributionsGPU.mode(Binomial(1, 0.51)) == 1
@test DistributionsGPU.mode(Binomial(1, 0.49)) == 0
