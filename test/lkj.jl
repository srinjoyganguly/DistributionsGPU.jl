...
@testset "LKJ integrating constant" begin
    ...
    η = 2.3
    lkj = LKJ(d, η)
    @test DistributionsGPU.lkj_vine_loginvconst(d, η) ≈ DistributionsGPU.lkj_onion_loginvconst(d, η)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.lkj_loginvconst_alt(d, η)
    @test lkj.logc0 == -DistributionsGPU.lkj_onion_loginvconst(d, η)
    #  =============
    #  odd uniform
    #  =============
    d = 5
    ...
    lkj = LKJ(d, η)
    @test DistributionsGPU.lkj_vine_loginvconst(d, η) ≈ DistributionsGPU.lkj_onion_loginvconst(d, η)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.lkj_onion_loginvconst_uniform_odd(d)
    @test DistributionsGPU.lkj_vine_loginvconst(d, η) ≈ DistributionsGPU.lkj_vine_loginvconst_uniform(d)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.lkj_loginvconst_alt(d, η)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.corr_logvolume(d)
    @test lkj.logc0 == -DistributionsGPU.lkj_onion_loginvconst_uniform_odd(d)
    #  =============
    #  even non-uniform
    #  =============
    d = 6
    ...
    lkj = LKJ(d, η)
    @test DistributionsGPU.lkj_vine_loginvconst(d, η) ≈ DistributionsGPU.lkj_onion_loginvconst(d, η)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.lkj_onion_loginvconst_uniform_even(d)
    @test DistributionsGPU.lkj_vine_loginvconst(d, η) ≈ DistributionsGPU.lkj_vine_loginvconst_uniform(d)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.lkj_loginvconst_alt(d, η)
    @test DistributionsGPU.lkj_onion_loginvconst(d, η) ≈ DistributionsGPU.corr_logvolume(d)
    @test lkj.logc0 == -DistributionsGPU.lkj_onion_loginvconst_uniform_even(d)
end

@testset "check integrating constant as a volume" begin
    #  d = 2: Lebesgue measure of the set of correlation matrices is 2.
    volume2D = 2
    @test volume2D ≈ exp( DistributionsGPU.lkj_onion_loginvconst(2, 1) )
    ...
    volume3D = 0.5π^2
    @test volume3D ≈ exp( DistributionsGPU.lkj_onion_loginvconst(3, 1) )
    ...
    volume4D = (32 / 27)*π^2
    @test volume4D ≈ exp( DistributionsGPU.lkj_onion_loginvconst(4, 1) )
