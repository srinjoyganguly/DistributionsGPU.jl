...
@testset "Testing fit for Bernoulli" begin
    for func in funcs, dist in (Bernoulli, Bernoulli{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.BernoulliStats)
...
@testset "Testing fit for Binomial" begin
    for func in funcs, dist in (Binomial, Binomial{Float64})
        ...
        @test isa(ss, DistributionsGPU.BinomialStats)
        @test ss.ns ≈ sum(x)
        @test ss.ne == n0
        @test ss.n == 100
...
@testset "Testing fit for Categorical" begin
    for func in funcs
        ...
        @test isa(ss, DistributionsGPU.CategoricalStats)
        @test ss.h ≈ h
...
@testset "Testing fit for Exponential" begin
    for func in funcs, dist in (Exponential, Exponential{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.ExponentialStats)
        @test ss.sx ≈ sum(x)
        @test ss.sw == n0
...
@testset "Testing fit for Normal" begin
    for func in funcs, dist in (Normal, Normal{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.NormalStats)
        @test ss.s  ≈ sum(x)
        @test ss.m  ≈ mean(x)
...
@testset "Testing fit for Normal with known moments" begin
    import DistributionsGPU.NormalKnownMu, DistributionsGPU.NormalKnownSigma
    μ = 11.3
    σ = 3.2
    ...
    for func in funcs
        ...
        @test isa(ss, DistributionsGPU.NormalKnownMuStats)
        @test ss.μ == μ
        ...
        @test isa(ss, DistributionsGPU.NormalKnownSigmaStats)
        @test ss.σ == σ
        @test ss.sx ≈ sum(x)
        @test ss.tw ≈ n0
...
@testset "Testing fit for Gamma" begin
    for func in funcs, dist in (Gamma, Gamma{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.GammaStats)
        @test ss.sx    ≈ sum(x)
...
@testset "Testing fit for Geometric" begin
    for func in funcs, dist in (Geometric, Geometric{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.GeometricStats)
        @test ss.sx ≈ sum(x)
        @test ss.tw ≈ n0
...
@testset "Testing fit for Poisson" begin
    for func in funcs, dist in (Poisson, Poisson{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.PoissonStats)
        @test ss.sx ≈ sum(x)
        @test ss.tw ≈ n0
...
@testset "Testing fit for InverseGaussian" begin
    for func in funcs, dist in (InverseGaussian, InverseGaussian{Float64})
        ...
        ss = suffstats(dist, x)
        @test isa(ss, DistributionsGPU.InverseGaussianStats)
        @test ss.sx    ≈ sum(x)
