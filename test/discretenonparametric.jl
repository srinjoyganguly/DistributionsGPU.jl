...
test_range(d)
vs = DistributionsGPU.get_evalsamples(d, 0.00001)
...
@test ss isa DistributionsGPU.DiscreteNonParametricStats{Int,Float64,Vector{Int}}
@test ss.support == [1,2,3,4,5]
@test ss.freq == [2., 3., 3., 2., 1.]

ss2 = suffstats(DiscreteNonParametric, xs, ws)
@test ss2 isa DistributionsGPU.DiscreteNonParametricStats{Int,Float64,Vector{Int}}
